...
class HrPayslipEmployees(models.TransientModel):
    ...
    def action_compute_sheet(self):
        ...
        for employee in self.env['hr.employee'].browse(data['employee_ids']):
            ...
            contract_id = self.env["hr.contract"].search([("employee_id","=",employee.id),("date_start","<=", to_date),
                                                          "|",("date_end",">=",from_date),("date_end","=",False)])
            
            #TODO: if need with account
            # journal_id = run_data.get('journal_id')[0] if not isinstance(run_data.get('journal_id'), int) else False
            res = {
                'employee_id': employee.id,
                'name': slip_data['value'].get('name'),
                'struct_id': slip_data['value'].get('struct_id'),
                'contract_id': slip_data['value'].get('contract_id') or contract_id and contract_id.id,
                'payslip_run_id': active_id,
                'input_line_ids': [(0, 0, x) for x in
                                   slip_data['value'].get('input_line_ids')],
                'worked_days_line_ids': [(0, 0, x) for x in 
                         slip_data['value'].get('worked_days_line_ids') or []],
                'date_from': from_date,
                'date_to': to_date,
