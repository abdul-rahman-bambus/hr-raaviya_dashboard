...
class HrPayslip(models.Model):
    ...
    basic_pay = fields.Float(string="Basic Wage", compute='_compute_pay', store=True)
    gross_pay = fields.Float(string="Gross Wage", compute='_compute_pay', store=True)
    net_pay = fields.Float(string="Net Wage", compute='_compute_pay', store=True)
    
    @api.depends("contract_id.wage", "line_ids")
    def _compute_pay(self):
        for record in self:
            if record.line_ids:
                basic_pay = record.line_ids.filtered(lambda l: l.code == 'BASIC')
                gross_pay = record.line_ids.filtered(lambda l: l.code == 'GROSS')
                net_pay = record.line_ids.filtered(lambda l: l.code == 'NET')
                record.basic_pay = basic_pay and basic_pay.amount
                record.gross_pay = gross_pay and gross_pay.amount
                record.net_pay = net_pay and net_pay.amount
            else:
                record.basic_pay = 0
                record.gross_pay = 0
                record.net_pay = 0
